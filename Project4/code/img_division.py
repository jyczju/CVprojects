import cv2
import numpy as np
import matplotlib.pyplot as plt

#分裂
def Division_Judge(img, h0, w0, h, w) :
    area = img[h0 : h0 + h, w0 : w0 + w]
    print(area)
    print(area.flatten())
    # mean = np.mean(area)
    std = np.std(area.flatten(), ddof = 1)
    print('std')
    print(std)

    if std > 15:
        return True
    else:
        return False

    # total_points = 0
    # operated_points = 0

    # for row in range(area.shape[0]) :
    #     for col in range(area.shape[1]) :
    #         if (area[row][col] - mean) < 2 * std :
    #             operated_points += 1
    #         total_points += 1

    # if operated_points / total_points < 0.95 :
    #     return True
    # else :
    #     return False

def Merge(img, h0, w0, h, w) :
    # area = img[h0 : h0 + h, w0 : w0 + w]
    # _, thresh = cv.threshold(area, 0, 255, cv.THRESH_OTSU + cv.THRESH_BINARY_INV)
    # img[h0 : h0 + h, w0 : w0 + w] = thresh
    for row in range(h0, h0 + h) :
        for col in range(w0, w0 + w) :
            if img[row, col] > 100 and img[row, col] < 200:
                img[row, col] = 0
            else :
                img[row, col] = 255

def Recursion(img, h0, w0, h, w) :
    #如果满足分裂条件继续分裂
    # cv2.imshow('img',img)
    if Division_Judge(img, h0, w0, h, w) and h>20 and w> 20 :
        #递归继续判断能否继续分裂
        #左上方块
        Recursion(img, h0, w0, int(h / 2), int(w / 2))
        #右上方块
        Recursion(img, h0, w0 + int(w / 2), int(h / 2), int(w / 2))
        #左下方块
        Recursion(img, h0 + int(h / 2), w0, int(h / 2), int(w / 2))
        #右下方块
        Recursion(img, h0 + int(h / 2), w0 + int(w / 2), int(h/ 2), int(w / 2))
    # else :
    #     #合并
    #     Merge(img, h0, w0, h, w)

def Division_Merge_Segmented() :
    img = cv2.imread('rice.png',0)
    origin_img = img.copy()
    cv2.imshow('origin_img',origin_img)
    # img = cv2.medianBlur(img, 5)
    # blur_img = img.copy()
    # img = cv2.equalizeHist(img)
    # equ_img = img.copy()

    
    Recursion(img, 0, 0, img.shape[0], img.shape[1])

    cv2.imshow('results',img)


    # plt.figure(figsize=(12, 4))
    # plt.subplot(141), plt.imshow(origin_img, cmap='gray')
    # plt.axis('off'), plt.title(f'$input\_image$')
    # plt.subplot(142), plt.imshow(blur_img, cmap='gray')
    # plt.axis('off'), plt.title(f'$blur\_image$')
    # plt.subplot(143), plt.imshow(equ_img, cmap='gray')
    # plt.axis('off'), plt.title(f'$equ\_image$')
    # plt.subplot(144), plt.imshow(img, cmap='gray')
    # plt.axis('off'), plt.title(f'$segmented\_image$')
    # plt.tight_layout()
    # plt.show()

    cv2.waitKey(0)

if __name__ == '__main__':
    Division_Merge_Segmented()
