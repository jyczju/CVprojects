...
class ImgNode():
    ...
    def node_split(self):
        ...
        sub_node1 = ImgNode(self.img, self, self.h0, int((self.h0+self.h1)/2), self.w0, int((self.w0+self.w1)/2)) # 创建子节点1
        sub_node2 = ImgNode(self.img, self, self.h0, int((self.h0+self.h1)/2), int((self.w0+self.w1)/2), self.w1) # 创建子节点2
        sub_node3 = ImgNode(self.img, self, int((self.h0+self.h1)/2), self.h1, self.w0, int((self.w0+self.w1)/2)) # 创建子节点3
        sub_node4 = ImgNode(self.img, self, int((self.h0+self.h1)/2), self.h1, int((self.w0+self.w1)/2), self.w1) # 创建子节点4
    ...
    def draw_node(self, img):
        ...
        thickness = 1 # 粗细
        lineType = 4 # 线型
        cv2.rectangle(img, (self.w0, self.h0), (self.w1, self.h1), point_color, thickness, lineType) # 绘制矩形
        return img
    ...
    def merge(self, region_img, threshold=5.0):
        ...
        for rn in self.right_node: # 遍历所有右侧节点
            if abs(self_mean - rn.cal_mean()) <= threshold and rn not in ImgNode.Visited_List: # 右侧节点与当前节点类似，且未被访问过
                # print('right', self_mean - rn.cal_mean())
                region_img = rn.merge(region_img, 5) # 对右侧节点进行递归合并

        for dn in self.down_node: # 遍历所有下方节点
            if abs(self_mean - dn.cal_mean()) <= threshold and dn not in ImgNode.Visited_List: # 下方节点与当前节点类似，且未被访问过
                # print('down', self_mean - dn.cal_mean())
                region_img = dn.merge(region_img, 5) # 对下方节点进行递归合并

        for un in self.up_node: # 遍历所有上方节点
            if abs(self_mean - un.cal_mean()) <= threshold and un not in ImgNode.Visited_List: # 上方节点与当前节点类似，且未被访问过
                # print('up', self_mean - un.cal_mean())
                region_img = un.merge(region_img, 5) # 对上方节点进行递归合并

        for ln in self.left_node: # 遍历所有左侧节点
            if abs(self_mean - ln.cal_mean()) <= threshold and ln not in ImgNode.Visited_List: # 左侧节点与当前节点类似，且未被访问过
